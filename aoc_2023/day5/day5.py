...
def solution_task_2():
    ...
    for row in data_input:
        row.strip()
        if 'seeds' in row:
            seeds_string = row.split(':')[1].strip().split(' ')
            starts = seeds_string[::2]
            ends = seeds_string[1::2]
            for s, e in zip(starts, ends):
                seeds.append(range(int(s),int(s)+int(e)))
        elif 'seed-to-soil' in row:
            current_map = 1
            continue
        elif 'soil-to-fertilizer' in row:
    ...
    for seed_range in [seeds[0]]:
        range_2 = [seed_range[0], seed_range[len(seed_range) // 2]]
        for seed in tqdm(range_2):
            a, b, c = check_in_map(seed_to_soil, seed)
            if a:
                soil = seed_to_soil[1][b][c]
            else:
